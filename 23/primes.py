...
# Code from http://www.rfc1149.net/blog/2006/02/10/prime-numbers-and-pell-equation/
...
def _gen_primes ():
    for i in xrange (3, sys.maxsize, 2):
        if _is_prime (i):
            _primes.append (i)
            yield i
...
def _gen_primes (minval):
    ...
    if minval:
        l = _primes[-1]
        if minval > l:
            while minval > l: l = next(_primary)
            i = len (_primes) - 1
        else:
            i = bisect.bisect_left (_primes, minval)
    for i in itertools.count (i):
        if i == len (_primes): yield next(_primary)
